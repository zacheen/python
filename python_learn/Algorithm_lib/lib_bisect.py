...
# bisect.bisect_left(ll, x, lo=0, hi=len(ll), *, key=None)
    ...
    # 在 ll 當中找到一個位置，讓 x 插入後 ll 仍然是排序好的。
        # 如果 ll 裡面已經有 x 出現，插入的位置會在所有 x 的前面（左邊）。回傳值可以被當作 list.insert() 的第一個參數
    # 參數 lo 和 hi 用來指定 list 中應該被考慮的子區間，預設是考慮整個 list 
    # key 指定帶有單個參數的 key function，用於從每個輸入元素中提取比較鍵。默認值為 None (直接比較元素)。

# bisect.bisect_right(ll, x, lo=0, hi=len(ll), *, key=None)
    # 回傳此項目在相同項目中最後一個位置"的後面" 
        # (所以這個位置並不是target 而是可以插入的位置)

# bisect.bisect(ll, x, lo=0, hi=len(ll), *, key=None)
